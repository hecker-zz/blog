...
- 说明：一般域管不会主动访问我们，我们可以 利用 Windows 打印系统远程协议(MS-RPRN)中的一种旧的但是默认启用的方法，在该方法中，域用户可以使用 MS-RPRN RpcRemotefindFirstpninterchangeNotification(Ex)方法强制任何运行了 Spooler 服务的计算机以通过 Kerberos 或 NTLM 对攻击者选择的目标进行身份验证。非约束性委派主机结合 Spooler 打印机服务漏洞，让域控机器  强制访问已控的具有本地管理员权限的非约束性委派机器0A，从而拿到域管理员的TGT，进而接管域控。(2008机器可能复现不了，因为版本的问题)
